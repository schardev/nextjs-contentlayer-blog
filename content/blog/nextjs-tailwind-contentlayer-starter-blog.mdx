...
The template comes with a default theme based on **Stone** color palette from Tailwind CSS. But don't worry, you have the freedom to create own theme by customising and tweaking `config.css`.
...
<Callout>
Image optimisations using custom `<Image/>{:jsx}` component only works if your blog is statically generated. **Do not use it as a client or server component.**
</Callout>
